...
def generate_blog():
    ...
    try:
        ...
        if contains_restricted_content(blog_english):
            return jsonify({'error': 'Content generation restricted for this topic.'}), 403
        if lang_ui != "English":
            translation_prompt = f"Translate this blog to {lang_ui}:\n\n{blog_english}"
        ...
        for kw in map(str.strip, keywords.split(",")):
            if len(kw) > 1 and kw.lower() not in blog_title.lower():
                blog_display = re.sub(fr"(?i)\\b({re.escape(kw)})\\b", r"<u><b>\\1</b></u>", blog_display)
        return jsonify({
